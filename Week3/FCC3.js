...
let terrier = new Dog('yoyo', 'black');
...
let ownProps = [];
// Only change code below this line
for (let ownProp in canary) {
  ownProps.push(ownProp);
}
...
myList.push(['Pen', 4], ['Computer', 1], ['Lamp', 3], ['Glasses', 1], ['speaker', 10]);
...
let myNestedArray = [
  // Only change code below this line
  [['unshift', false, 1, 2, 3, 'complex', 'nested', [['deepest'], 'deeper'], 'deep']],
...
filteredList = watchList.filter(item => item.imdbRating >= 8.0);
...
function getRating(watchList) {
  ...
  averageRating = filtered.reduce((acc, item) => acc + Number(item.imdbRating), 0) / filtered.length;
...
const squareList = arr => {
  // Only change code below this line
  arr = arr.filter(item => item % 1 === 0 && item > 0);
